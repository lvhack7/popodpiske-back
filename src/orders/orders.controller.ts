...
export class OrdersController {
    ...
    @ApiOperation({ 
        ...
        description: 'Добавляет платеж к заказу, обновляя его статус и связанные данные.' 
    })
    @ApiBody({ schema: { example: { orderId: 'abc123-uuid' } } })
    @ApiResponse({ status: 200, description: 'Платеж успешно добавлен к заказу.' })
